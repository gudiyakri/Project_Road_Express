...
router.get("/addNumber", ensureAuthenticatedAdmin(), async (req, res) => {
	try {
		const data = await pool.query("SELECT * from sim_tracking");
		return res.render("admin/addNumber", {
			number: data,
		});
	} catch (error) {
		console.error(error);
...
router.get("/checkConcent", ensureAuthenticatedAdmin(), async (req, res) => {
	try {
		// console.log(req)
		let tels = req.query.tel;

		superagent
			.get(`https://dashboard.traqo.in/api/v2/check_consent/`)
			.set({
				"Accept": "application/json",
				"Content-Type": "application/json",
			})
			.auth("9123402582", "9123402582@123")
			.send({ tel: tels })
			.end((error, response) => {
				console.log(response.text);
				req.flash("success_msg", "Concent Has been Updated");
				return res.redirect("/simTracking/addNumber");
			});
		// .then(async (response) => {
...
router.post("/resndConcent", ensureAuthenticatedAdmin(), async (req, res) => {
	try {
		const data = { phone_number: req.body.phone_number };
		axios
			.post(`https://dashboard.traqo.in/api/v2/resend_consent_sms/`, data, {
				headers: {
					"Content-Type": "application/json",
				},
				auth: {
					username: "9123402582",
					password: "9123402582@123",
				},
			})
			.then(async response => {
				// your action after success
...
router.post("/addNumber", ensureAuthenticatedAdmin(), async (req, res) => {
	try {
		...
		form.parse(req, async (error, fields, files) => {
			...
			const transDetails = await pool.query("SELECT mob FROM sim_tracking WHERE mob = ?", [
				fields.phone_number,
			]);
			if (transDetails.length) {
				req.flash("error_msg", "Number is already registered");
				return res.redirect("/simTracking/addNumber");
			} else {
				console.log(fields);
				const data = {
				...
				await pool.query("INSERT INTO sim_tracking (name,mob,operator) VALUES (?,?,?)", [
					data.name,
					data.phone_number,
					data.operator,
				]);
				axios
					.post(`https://dashboard.traqo.in/api/v2/add_phone_number/`, data, {
						headers: {
							"Content-Type": "application/json",
						},
						auth: {
...
// 				};
// 				await pool.query(
// 					"INSERT INTO driver_tab (Did,d_name,phn,prof_pic,aadhar_front_pic,dl_pic_front,dl_pic_back,verif_flag,city,state,birthday,tid,add_by,bankAccd,u_flag,lice_exp,acc_name,ifsc) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
// 					[
// 						driver.did,
// 						driver.d_name,
// 						driver.phn,
